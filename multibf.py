...
try:
	...
	tampil('\rm[!]Sepertinya Kamu Belum Install Module \rcmechanize\rm')
	os.sys.exit()
def keluar():
	simpan()
	tampil('\rm[!]Keluar')
...
def saring_id_group0():
	...
	while 1:
		...
		tampil('\rh[*]Sedang Mengecek Group....')
		...
		try:
			...
			tampil('\rm[!]Id yang kamu masukan salah')
			continue
...
def idgroup():
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Terambil \rh %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?]Wanna Crack (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm[!]Terambil \rp%d id'%len(id_bteman))
		else:
			tampil('\rm[!]Batal')
			keluar()
	while 1:
...
def crack(d):
	...
	if i.upper() == 'P':
		...
		for i in D:
			...
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Tidak Puas dengan Hasil Tadi,Mau coba lagi (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
def crack0(data,sandi,p):
	tampil('\rh[*]Crack \rk%d Akun \rhdengan sandi \rm[\rk%s\rm]'%(len(data),sandi))
	...
	tampil('\rh[*]Jumlah akun yg berhasil\rp      %d'%len(akun_sukses))
	tampil('\rm[*]Jumlah akun yg gagal\rp         %d'%len(akun_gagal))
	tampil('\rk[*]Jumlah akun yg cekpoint\rp      %d'%len(akun_cekpoint))
	tampil('\rc[*]Jumlah akun yg error\rp         %d'%len(akun_error))
	if p:
		i = inputD('[?]Tidak Puas dengan Hasil Tadi,Mau coba lagi (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(data)
...
def menu():
	tampil('''\rh
            ...
            \rk*   *\rh /+/++//
             \ /  |/__//
           {\rmX\rh}v{\rmX\rh}|\rcMr11pew\rh|==========.
             [']  /'|'\           \\
                 /  \  \           '
                 \_  \_ \_    \rk*\rhBrigadeBrother
\rk###########################################################
#             \rb*SECOND GENERATION BRUTEFORCE FACEBOOK*\rk                 #
# \rhBY\rp                                             Mr11pew \rk#
# \rhGroup FB\rp  https://m.facebook.com/groups/614128728958126 \rk#
# \rhGitHub\rp                      https://github.com/Mr11pew \rk#
#       \rmSAVE THE NEXT GENERATION FROM CRIMINAL ACCOUNT        \rk#
###########################################################''')
